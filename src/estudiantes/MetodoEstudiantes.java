...
public void imprimirEstudiantes () {
	...
	for (int i = 0; i < nombres.length; i++) {
		System.out.println("estudiante "+nombres[i]+" de edad "+edad[i]);
	}
}
...
public void estudiantesMayores () {
	...
	for (int i = 0; i < edad.length; i++) {
		if(edad[i] >=(mayor)) {
			 System.out.println("El estudiante mayor de edad " + nombres[i] + " tiene " + edad[i] + " años");		
		}
	}
}
...
public void buscarEstudiantePorNombre () {
	...
	for (int i = 0; i < nombres.length; i++) {
		if(nombres[i].equalsIgnoreCase(nombreBuscar)) {
			System.out.print("Lo encontro en la pos: "+i);
			contador ++;
		}
	}
	
	if (contador>0) {
		System.out.println("Lo encontro "+contador+" veces");
	}else {
		System.out.println("No encontro el nombre "+nombreBuscar);
	}
}

public void buscarEstudiantePorEdad() {
	int edadBuscar = Integer.parseInt(JOptionPane.showInputDialog("ingresa la edad del estudiante a buscar"));
	
	for (int i = 0; i < edad.length; i++) {
		if(edad[i]==edadBuscar) {
			System.out.println("la edad ingresada "+edad[i]+" corresponde a los alumnos "+nombres[i]);
		}
	}
